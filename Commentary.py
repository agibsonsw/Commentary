...
PACKAGE_SETTINGS = "Commentary.sublime-settings"
...
OUTLINED = sublime.DRAW_OUTLINED if \
    sublime.load_settings(PACKAGE_SETTINGS).get("use_outline", True) else 0
# whether to use an outline, or block, when highlighting comments
# (default) True == use outline

ICON = os.path.pardir + '/Commentary/icon' if \
...
"""
    ...
    #divComments { display: none; height: 400px; width: 540px;
    ...
    #divComments.overlay #bottom_row, #divComments.overlay_bl #bottom_row {
        display: table-row;
    }
    #tblComments {
        box-shadow: 5px 5px 5px rgba(0, 0, 0, 0.1);
...
CKBs_COMMENTS = \
"""
<p>Show table of comments:<input type="checkbox" name="ckbComments"
id="ckbComments" value="1" onclick="listComments()">&nbsp;
Overlay table of comments:<input type="checkbox" name="ckbOverlay"
id="ckbOverlay" value="1" onclick="overlayComments()">&nbsp;
Show/hide comments (disables hover):<input type="checkbox" name="ckbToggle"
id="ckbToggle" value="1" onclick="toggleComments()"></p>
"""
...
SCOPEDCOMMENT = \
"""<a class="tooltip" href="#" onclick="return false;">%(scoped)s<span class=
"comment"><span class="stamp">%(stamp)s</span>%(comment)s</span></a>"""
...
COMMENTS_TBLEND = \
"""
    <tr id="bottom_row"><td colspan="3" style=\"font-size:smaller; text-align:right;\">
    <a href="#top">Top</a>&nbsp;&nbsp;<a href="#" onclick="listComments();return false;">Close</a>
    &nbsp;&nbsp;Position: bottom-left <input type="checkbox" name="ckbBottom" 
    id="ckbBottom" value="1" onclick="tableToBottom()"></td>
    </tr>
</table>
</div>
"""
...
class CommentaryCommand(sublime_plugin.TextCommand):
    ...
    def check_suitability(self, word):              # returns (unsuitable, unsuitable_err)
        if len(word) < 2:
            ...
            return (True, "Place cursor on a 'sensible' word (not starting with a number): %s" \
                % word)
        else:
    ...
    def same_word(self, key_pt):                    # is the comment-point still on the same word?
        try:                                        # key_pt should/will be pointing at the  
            word_region = self.view.word(key_pt)    # beginning of the comment's word.
    ...
    def adjust_comments(self):      # utility fn - move all comment-pts to beginning of their current
        ...
        for key_pt in [ pt for pt in self.view.vcomments.keys() if pt < eov ]:
            ...
            self.view.vcomments[current['word_pt']] = \
                (existing_wd, comment, current['line'], dt_stamp())
    ...
    def select_comments(self):              # clears current selection and highlights, and deletes
        ...
        for key_pt in sorted(self.view.vcomments.keys()):
            ...
            if key_pt >= eov:                       # delete comments past end of the view
                ...
                print "Comment past end-of-view deleted: %s (was line %d)" % \
        ...
        if not sels:                    # nothing selected, so re-instate original (1st) 
            sels.add(sel_orig)          # selection - or beginning of view
    ...
    def select_next(self, direction = 'down', start_at = -99):  # start at a certain pt in view
        ...
        if direction == 'down':
            sorted_pts = (key_pt for key_pt in sorted(self.view.vcomments.keys()) \
                if key_pt > curr_pt)
        else:
            sorted_pts = (key_pt for key_pt in reversed(sorted(self.view.vcomments.keys())) \
                if key_pt < curr_pt)
        try:
    ...
    def push_comments(self, direction = 'down'):            # move selected comment(s) down or up
        ...
        else:               # pushing comments within selection
            pt_begin, pt_end = (curr_sel.begin(), curr_sel.end())
        if pt_begin == pt_end:
        ...
        for next_pt in sorted_pts:
            ...
            if new_region:                          # was the comment-word found further up/down?
                ...
                if new_region_begin in self.view.vcomments:
                    ...
                    print "Already a comment at line %d comment: %s" % \
                        (new_comment_line + 1, old_comment)
                    return "Already a comment at line %d comment: %s" % \
                        (new_comment_line + 1, old_comment)
                self.view.vcomments[new_region_begin] = \
                    (prev_wd, prev_comment, new_comment_line, dt_stamp())
    ...
    def pull_comment(self, direction = 'down'):     # pull next or previous comment to cursor
        selection = self.get_metrics()
        if not selection:
        ...
        self.view.vcomments[curr_pt] = \
            (selection['word'], next_comment, selection['line'], dt_stamp())
    ...
    def add_comment(self, text, code = False):                  # at cursor position
        ...
        if curr['word_pt'] in self.view.vcomments and \
            ...
            self.add_highlight(curr['word_region'], False)      # False == not an error region
            self.just_added = True                              # don't re-display comment-text
    ...
    def save_comments(self):            # the same filename/location, with 'cmts' added at end
        ...
        if fname == "Untitled.":                    # getcwd() - 'current working directory'
            print "File not saved, so comments saved as: %s%sUntitled.cmts" % \
                (os.getcwd(), path.sep)
            return "File not saved, so comments saved as: %s%sUntitled.cmts" % \
                (os.getcwd(), path.sep)
        else:
    ...
    def process_commentary(self, text, caller_id):                  # on_done for comments panel
        ...
        elif comment_command in ('PREV','PREVIOUS','UP','SELECT' 'PREV'):   # select previous comment
...
class PrintHtmlCommand(sublime_plugin.TextCommand):
    def setup(self, numbers):
        ...
        if curr_sel.empty() or len(self.view.lines(curr_sel)) < 2:      # don't print just 1 line
            ...
            self.pt, self.end, self.curr_row, self.partial = (0, 1, 1, False)
            # partial = False: print entire view
        else:
    ...
    def write_header(self, the_html):
        ...
        the_html.write(('\tli { color: %s; margin-top: %dpt; margin-bottom: %dpt; }\n' \
            % (self.gfground, self.padd_top, self.padd_bottom)).encode(*UTF8))
        ...
        the_html.write((JS_STANDARD).encode(*UTF8))
        if self.has_comments:
            the_html.write((JS_COMMENTS).encode(*UTF8))  # JS code to display comments
    ...
    def convert_view_to_html(self, the_html):
        first_line = True
        if self.has_comments:
            self.comments_list = []     # [(line number, comment, stamp),..] for the commments-table
        ...
        for line in self.view.split_by_newlines(sublime.Region(self.pt, self.size)):
            ...
            while self.end <= self.line_end:
                ...
                tidied_text = entity_ref(tidied_text)
            ...
            if temp_line:
                ...
                for (the_colour, tidied_text, the_comment, the_stamp) in temp_line:
                    ...
                    if the_comment is not None:
                        ...
                        self.comments_list.append((line_no, the_comment, the_stamp))    # for comments' table
                    html_line += the_span
                the_html.write((html_line + '</li>').encode(*UTF8))
                temp_line[:] = []
            elif first_line:
                the_html.write(('\n</li>').encode(*UTF8))
            first_line = False
    ...
    def write_body(self, the_html):
        ...
        temp_body += '</div><pre id="preCode"><ol id="olCode"><li value="%d">' % \
            (self.curr_row)    # use code's line numbering
        
        the_html.write((temp_body).encode(*UTF8))
    ...
    def run(self, edit, numbers):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.html') as the_html:
            ...
            except Exception:                                   # .. otherwise, open in ST tab
